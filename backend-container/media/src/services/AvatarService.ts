...
/** Ruta legacy que el gateway expone al móvil (`/api/auth/avatars/:id`). */
export function legacyClientAvatarPath(userId: string): string {
...
export function resolveAvatarUrl(userId: string, storedUrl?: string): string | undefined {
  if (storedUrl?.startsWith('/api/media/avatars/') || storedUrl?.startsWith('/api/auth/avatars/')) return storedUrl;
