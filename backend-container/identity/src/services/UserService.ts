...
export async function loginUser(login: string, password: string): Promise<UserDocument> {
  const key = login.trim();
  if (!key) throw new Error('invalid_credentials');
  const byEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(key);
  const doc = await users().findOne({
    isActive: true,
    ...(byEmail ? { email: key.toLowerCase() } : { username: key }),
  });
...
export async function updateUserAvatarUrl(userId: string, avatarUrl: string | null): Promise<void> {
  if (!ObjectId.isValid(userId)) throw new Error('invalid_user');
  await users().updateOne(
    { _id: new ObjectId(userId) },
