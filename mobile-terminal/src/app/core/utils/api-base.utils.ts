...
export function resolveApiBase(): string {
  ...
  if (typeof window !== 'undefined' && !getStoredApiUrl()) {
    try {
      ...
      // LAN: sustituir localhost por IP de la página (ionic serve en 192.168.x.x)
...
export function networkErrorMessage(): string {
  ...
  if (base.includes('ngrok') || getStoredApiUrl()) {
    return 'No se pudo contactar al servidor. Verifica que ngrok apunte al puerto 3000 y que la URL del backend esté bien configurada.';
  }
