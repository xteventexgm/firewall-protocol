/**
 * Colección `auth_sessions` — refresh tokens (DATABASE.md §10.2).
 */
...
export async function createAuthSession(userId: string, deviceId?: string): Promise<{ refreshToken: string; expiresAt: Date }> {
...
export async function rotateAuthSession(refreshToken: string): Promise<{ userId: string; newRefreshToken: string } | null> {
