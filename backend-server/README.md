> **Monorepo:** [`../README.md`](../README.md) · **Contrato socket:** [`../SOCKET_CONTRACT.md`](../SOCKET_CONTRACT.md) · **Changelog:** [`../CHANGELOG.md`](../CHANGELOG.md)  
> **Tipos canónicos:** [`src/types/events.types.ts`](src/types/events.types.ts)

# Firewall Protocol — Backend

Servidor realtime del juego **Firewall Protocol**: thriller de deducción social multijugador (5–15 jugadores) con temática de ciberseguridad. Coordina salas, fases, roles, acciones nocturnas, votación diurna, chat multicanal y victoria en tiempo real vía WebSockets.
...
- Voto en blanco (`target: null`) bajo clave `skip`
- Phisher redirige **votos diurnos** en secreto
- **`maybeEndGame()`** al eliminar por voto o honeypot drag — victoria sin fase extra de verificación
- En `VERIFICACION`, avanzar fase comprueba victoria antes de pasar a `NOCHE`
- **Victoria por bando**: System (0 hackers) / Black Hat (H > S)
- **Victoria solitaria**: Troll, Gusano, Minero; Zero-Day hereda bando asumido

### Chat (`ChatManager`)
- Canales: `public`, `dead` (espectadores), `hacker` (Black Hat)
- Muertos: solo `dead`; vivos no pueden usar canal dead
- Hackers vivos: `hacker` en `NOCHE` y fases públicas
- Muertos pueden chatear en `NOCHE` y `VERIFICACION`
- Rate-limit: 3 s entre mensajes, burst 10/min por jugador
...
- `nightResolved` — resolución completa en `/dashboard`; payload **público** (sin `logs` ni `privateResults`) en `/game`
- `publicLog` — entradas SIEM anonimizadas (sin nombres de rol internos en dashboard)
- `gameStats` / MVP al terminar partida
...
### Persistencia
- MongoDB 6 cuando `MONGO_URI` está definido (`games`, `roles`, `session_logs`)
- JSON en disco vía `dbSyncService` como fallback (`data/games/<roomId>.json`)
- Caché hidratada al arranque y cola ordenada de escrituras MongoDB
- Setup, migración y seed con `npm run db:setup`, `npm run db:migrate` y `npm run db:seed`
- `DATA_DIRECTORY` y `MONGO_DB_NAME` configurables desde `.env`
...
│   ├── ChatManager.ts       # Chat multicanal y rate-limit
│   ├── NightOrchestrator.ts # Orquestación nocturna y minijuegos
│   ├── PublicLogService.ts  # Logs públicos anonimizados
│   ├── MinigameChallengeManager.ts
...
| **VERIFICACIÓN** | Comprueba victoria al avanzar; si no hay ganador → `NOCHE` |
| **FIN** | Partida terminada; emite `gameOver` |

Flujo cíclico: `VERIFICACIÓN → NOCHE` si nadie ha ganado. La victoria por voto puede saltar directo a `FIN` sin pasar por verificación.
...
Levanta Node 18 con el backend en el puerto 3000 y MongoDB 6 en el puerto 27017. El volumen `mongodb_data` conserva los datos entre reinicios.

Para crear colecciones/índices, importar los JSON existentes y poblar el catálogo de 44 roles:
